...
export async function shutdownWithTelemetry({
  closeServer,
  shutdownTelemetry,
  timeoutMilliseconds = 2_000,
}: {
  closeServer: () => void | Promise<void>;
  shutdownTelemetry: () => void | Promise<void>;
  timeoutMilliseconds?: number;
}): Promise<void> {
  try {
    await runWithin(closeServer, timeoutMilliseconds);
  } finally {
    await runWithin(shutdownTelemetry, timeoutMilliseconds);
  }
}
