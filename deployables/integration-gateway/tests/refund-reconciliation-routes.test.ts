...
test('reconciliation finds an authoritative completed refund without issuing another', async (context) => {
  ...
  const response = await app.inject({ method: 'POST', url: '/internal/v1/refund-reconciliations', headers: { [WORKFLOW_ACCESS_ASSERTION_HEADER]: 'worker-reconcile-assertion' }, payload: { orderReference: 'ORDER-123', amount: { amountMinor: 5_000, currency: 'USD' } } });
