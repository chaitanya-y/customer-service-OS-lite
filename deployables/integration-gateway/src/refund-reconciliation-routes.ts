...
const requestSchema = z.object({ orderReference: z.string().trim().min(1).max(100), amount: z.object({ amountMinor: z.number().int().positive(), currency: z.literal('USD') }).strict() }).strict();

/** Read-only recovery lookup for an action whose provider response was uncertain. */
export function registerRefundReconciliationRoutes(app: FastifyInstance, commerceProvider: CommerceProvider, verify?: VerifyWorkflowAccessAssertion): void {
  app.post('/internal/v1/refund-reconciliations', async (request, reply) => {
    ...
    return refund ? { status: 'SUCCEEDED', providerRefundId: refund.id } : { status: 'NOT_FOUND' };
  });
}
